...
public class FokCacheTrie {
	...
	private FokTrieCache cache;
	private ICryptoHandler crypto;
	private Node root;
	private boolean async = true;

	public FokCacheTrie(ICryptoHandler crypto) {
		this.crypto = crypto;
		this.cache = new FokTrieCache();
	}
	...
	public boolean flush() {
		if (root != null && root.dirty) {
			...
			encode();
			// TODO save to db
			// for (KeySet<byte[]> b : cache.storage.keySet()) {
			//
			// }
	...
	public boolean equals(Object o) {
		...
		FokCacheTrie trieImpl1 = (FokCacheTrie) o;

		return BytesComparisons.equal(getRootHash(), trieImpl1.getRootHash());
