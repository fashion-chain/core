...
public final class FokTrieKey {
	...
	public static FokTrieKey fromNormal(byte[] key) {
		return new FokTrieKey(key);
	}

	public static FokTrieKey fromPacked(byte[] key) {
		return new FokTrieKey(key, ((key[0] >> 4) & ODD_OFFSET_FLAG) != 0 ? 1 : 2, ((key[0] >> 4) & TERMINATOR_FLAG) != 0);
	}

	public static FokTrieKey empty(boolean terminal) {
		return new FokTrieKey(BytesHelper.EMPTY_BYTE_ARRAY, 0, terminal);
	}

	public static FokTrieKey singleHex(int hex) {
		FokTrieKey ret = new FokTrieKey(new byte[1], 1, false);
		ret.setHex(0, hex);
		return ret;
	}

	public FokTrieKey(byte[] key, int off, boolean terminal) {
		this.terminal = terminal;
		this.off = off;
		this.key = key;
	}

	private FokTrieKey(byte[] key) {
		this(key, 0, true);
	}
	...
	public FokTrieKey shift(int hexCnt) {
		return new FokTrieKey(this.key, off + hexCnt, terminal);
	}

	public FokTrieKey getCommonPrefix(FokTrieKey k) {
		...
		FokTrieKey ret = new FokTrieKey(prefixKey, (prefixLen & 1) == 0 ? 0 : 1,
	...
	public FokTrieKey matchAndShift(FokTrieKey k) {
		// TODO unknown null
		if (k == null) {
			return null;
		}
	...
	public FokTrieKey concat(FokTrieKey k) {
		if (isTerminal())
		...
		FokTrieKey ret = new FokTrieKey(newKeyBytes, newLen & 1, k.isTerminal());
		for (int i = 0; i < len; i++) {
	...
	public boolean equals(Object obj) {
		if (obj == null)
			return false;
		if (obj instanceof FokTrieKey) {
			FokTrieKey k = (FokTrieKey) obj;
			int len = getLength();
